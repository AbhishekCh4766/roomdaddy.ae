...
if($date!="")
{
	$where.=" `fld_date` like '$date%' AND ";
}
if($expense_on!="")
{
	$where.=" `fld_expense_on` = '$expense_on' AND ";
}
if($expense_type!="")
{
	$where.=" `fld_expense_type` = '$expense_type' AND ";
}
if($payment_by!="")
{
	$where.=" `fld_payment_by` = '$payment_by' AND ";
}
if($payment_to!="")
{
	$where.=" `fld_payment_to` = '$payment_to' AND ";
}

if($_SESSION['Enron FZE']['userid'] !='SuperAdmin')
{
if($_SESSION['Enron FZE']['userid']!="")
{
   $admin_id = $_SESSION['Enron FZE']['userid'];
   $where.="`fld_assign_admin_id` = '$admin_id' AND ";
}
}
...
foreach($getExpense as $expense)
{
	...
	if($getExpense[0]!="")
	{
		...
		<tr>
			...
			<td>
				<span style="cursor:pointer;"><i class="fa fa-edit"><a href="<?=SERVER_PATH?>expense.php?eid=<?=base64_encode($expense['fld_id'])?>"></a></i></span>
			</td>
		</tr>
		<?php
	}
