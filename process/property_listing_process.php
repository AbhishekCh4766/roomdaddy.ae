...
if(empty($_REQUEST['buildingid']))
{
	echo "<tr><td colspan='2'>No Record Foundggg</td></tr>";
	exit;
}
...
if($fleets)
{
	...
	{
		// $getTanentRent = $db->getTanentById($room['fld_tanent_id']);
		// print_r($getTanentRent);
		// die;
		?>
		<tr>
			<td>
			...
			<td>
				<?php
				$checkStatus=$db->checkCurrentMonthRentStatusByID($room['fld_id'],$room['fld_tanent_id']);
				$getTanentRent = $db->getTanentById($room['fld_tanent_id']);
				if($room['fld_is_rented']!=0)
				{
					if($checkStatus[0]=="")
					{
						echo "<a href='pay_rent.php?roomid=".base64_encode($room['fld_id'])."&tanent_id=".base64_encode($getTanentRent[0]['fld_id'])."'>Not Paid</a>";
					}
					else
					{
						$sum=0;
