...
$date=$_REQUEST['rent_collection_year']."-".$_REQUEST['rent_collection_month']."-".$_REQUEST['rent_collection_date'];
if($_REQUEST['rent']=="")
{
	$error_msg.='<span class="ico_cancel" id="error_msg">Please Enter Rent.</span>';
}
$roomid=base64_decode($_REQUEST['roomid']);
$getRoomInfo=$db->getRoomsById($roomid);
//if($_REQUEST['rent_id']=='0' || $_REQUEST['rent_id']==0)
{
	if($error_msg=="")
	{
		$flag = $db->addRent(
							...
							//	SG_Validate_Input($getRoomInfo[0]['fld_tanent'],INPUT_TYPE_TEXT),
								SG_Validate_Input($getTanentInfo[0]['fld_id'],INPUT_TYPE_TEXT),
								...
								SG_Validate_Input($_REQUEST['rent_collect_by'],INPUT_TYPE_TEXT),
